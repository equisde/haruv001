...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // It is common for nodes with good haru times to suddenly become lagged,
    // due to a new block arriving or other large transfer.
    // Merely reporting harutime might fool the caller into thinking the node was still responsive,
    // since harutime does not update until the haru is complete, which might take a while.
    // So, if a haru is taking an unusually long time in flight,
    // the caller can immediately detect that this is happening.
    int64_t nHaruUsecWait = 0;
    if ((0 != nHaruNonceSent) && (0 != nHaruUsecStart)) {
        nHaruUsecWait = GetTimeMicros() - nHaruUsecStart;
    }

    // Raw haru time is in microseconds, but show it to user as whole seconds (Bitcoin users should be well used to small numbers with many decimal places by now :)
    stats.dHaruTime = (((double)nHaruUsecTime) / 1e6);
    stats.dHaruWait = (((double)nHaruUsecWait) / 1e6);
}
#undef X
    ...
    while (true)
    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            if (nTime - pnode->nTimeConnected > 60)
            {
                ...
                else if (pnode->nHaruNonceSent && pnode->nHaruUsecStart + TIMEOUT_INTERVAL * 1000000 < GetTimeMicros())
                {
                    printf("haru timeout: %fs\n", 0.000001 * (GetTimeMicros() - pnode->nHaruUsecStart));
                    pnode->fDisconnect = true;
                }
            }
        }
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Haru " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    {"nodo.scbtccoin.com", "nodo.scbtccoin.com"},
    {"nodo2.scbtccoin.com", "nodo2.scbtccoin.com"},
    {"nodo3.scbtccoin.com", "nodo3.scbtccoin.com"},
    {"nodo.cryptoharu.com", "nodo.cryptoharu.com"},
    {"nodo2.cryptoharu.com", "nodo2.cryptoharu.com"}
};
...
{
    // Make this thread recognisable as the address dumharu thread
    RenameThread("haru-adrdump");
...
void ThreadOpenAddedConnections2(void* parg)
{
    ...
    for (unsigned int i = 0; true; i++)
    {
        ...
        // (keeharu in mind that addnode entries can have many IPs if fNameLookup)
        {
            LOCK(cs_vNodes);
            BOOST_FOREACH(CNode* pnode, vNodes)
...
void ThreadMessageHandler2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        // we're sleeharu, but we must always check fShutdown after doing this.
        vnThreadsRunning[THREAD_MESSAGEHANDLER]--;
        if (fSleep)
            MilliSleep(100);
        if (fRequestShutdown)
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Haru is probably already running."), addrBind.ToString().c_str());
        else
...
void RelayForTunaElectionEntryHaru(const CTxIn vin, const std::vector<unsigned char> vchSig, const int64_t nNow, const bool stop)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
...
void SendForTunaElectionEntryHaru(const CTxIn vin, const std::vector<unsigned char> vchSig, const int64_t nNow, const bool stop)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
