...
enum AvailableCoinsType
{
    ...
    ONLY_NONDENOMINATED_NOTMN = 4 // ONLY_NONDENOMINATED and not 5000 HARU at the same time
};
...
public:
    ...
    std::set< std::set<CTxDestination> > GetAddressGrouharus();
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nHARU, int64_t& nAnon, const isminefilter& filter) const
    {
        nHARU = 0;
        nAnon = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    mutable int64_t nCredHARUCached;
    ...
    bool GetCredit(int64_t& nCredHARU, int64_t& nCredAnon, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredHARU = nCredAnon = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredHARUCached = 0;
            nCredAnonCached = 0;
            pwallet->GetCredit(*this, nCredHARUCached, nCredAnonCached, ISMINE_SPENDABLE);
            fCreditSplitCached = true;
        };

        nCredHARU = nCredHARUCached;
