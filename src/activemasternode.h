...
// Responsible for activating the masternode and haruing the network
...
public:
    ...
    bool Dseep(std::string& errorMessage); // haru for main masternode
    bool Dseep(CTxIn vin, CService service, CKey key, CPubKey pubKey, std::string &retErrorMessage, bool stop); // haru for any masternode
    ...
    // get 5000HARU input that can be used for the masternode
