...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if (pindexBest->nHeight == 1)
		...
		nSubsidy = 0.2 * COIN;
	else if (pindexBest->nHeight <= 3398) // V1.0.0 fork in block 3398/ Block 3398 ~ 6,796k HARU
		nSubsidy = 0.01 * COIN;
	else if (pindexBest->nHeight <= 1000000) // V1.0.0 / Block 1m ~ 993,204k HARU
		nSubsidy = 0.2 * COIN;	
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    */
    ...
    // ----------- masternode payments -----------
    // Once upon a time, People were really interested in HARU.
    // So much so, People wanted to bring HARU to the moon. Even Mars, Sooner than the roadster...
    ...
    // People wanted masternodes... Really Bad. But King Carsen was already busy with the rest of HARU
        ...
        if(IsProofOfStake() && pindex != NULL){
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (pindex != NULL)
                {
                        ...
                        if(vtx[1].vout[i].scriptPubKey == payee )
                            ...
                            BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                            {
                                ...
                                if (vtx[1].vout[i].scriptPubKey == pubScript)
                                {
                                    ...
                                    if (fDebug) printf("Masternode PoS payee found at block %d: %s who got paid %s HARU (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[1].vout[i].nValue / COIN).c_str(), paidAge, mn.nBlockLastPaid);
                    ...
                    if(fDebug) { printf("CheckBlock-POS() : Is initial download, skipharu masternode payment check %ld\n", pindexBest->nHeight+1); }
                }
            } else {
                if(fDebug) { printf("CheckBlock-POS() : Skipharu masternode payment check - nHeight %ld Hash %s\n", pindexBest->nHeight+1, GetHash().ToString().c_str()); }
            }
            ...
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (pindex != NULL)
                {
                    ...
                    for (unsigned int i = 0; i < vtx[0].vout.size(); i++) {
                        ...
                        BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                        {
                            ...
                            if (vtx[0].vout[i].scriptPubKey == pubScript)
                            {
                                int lastPaid = mn.nBlockLastPaid;
                                int paidAge = pindex->nHeight+1 - lastPaid;
                                if (fDebug) printf("Masternode PoW payee found at block %d: %s who got paid %s HARU (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[0].vout[i].nValue).c_str(), paidAge, mn.nBlockLastPaid);
                                if (paidAge < 150) // TODO: Probably make this check the MN is in the top 50?
                                {
                                    if (fDebug) printf("WARNING: This masternode payment is too aggressive and will not be accepted in v3+\n");
                                }
                    ...
                    if(fDebug) { printf("CheckBlock-POW() : Is initial download, skipharu masternode payment check %ld\n", pindexBest->nHeight+1); }
                }
            } else {
                if(fDebug) { printf("CheckBlock-POW() : Skipharu masternode payment check - nHeight %ld Hash %s\n", pindexBest->nHeight+1, GetHash().ToString().c_str()); }
            }
        }

         else {
            if(fDebug) { printf("CheckBlock() : pindex is null, skipharu masternode payment check\n"); }
        }
    } else {
        if(fDebug) { printf("CheckBlock() : skipharu masternode payment checks\n"); }
    }
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "https://www.coindesk.com/microsoft-is-pushing-new-blockchain-id-products-but-theres-pushck-too/";
        CTransaction txNew;
        txNew.nTime = 1539367364;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x8a6f632d3965cfb5900271a7caf9acbf59932532e9fd5a59b3ffaf8722dc0d78"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb2, 0x9e, 0xe1, 0xb4 };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    {
        printf("dropmessagestest DROPHARU RECV MESSAGE\n");
        return true;
    }
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            {
                if (fDebugNet) printf("  getblocks stopharu at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(0,20).c_str());
            ...
            if (--nLimit <= 0)
            {
                ...
                if (fDebugNet) printf("  getblocks stopharu at limit %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(0,20).c_str());
    ...
    else if (strCommand == "haru")
    {
        if (pfrom->nVersion > BIP0031_VERSION)
        {
            ...
            //    is overloaded it won't respond to harus quickly and the remote node can
            //    avoid sending us more work, like chain download requests.
            //
            // The nonce stops the remote getting confused between different harus: without
            // it, if the remote node sends a haru once per second and this node takes 5
            // seconds to respond to each, the 5th haru the remote sends would appear to
            // return very quickly.
    ...
    {
        int64_t haruUsecEnd = nTimeReceived;
        uint64_t nonce = 0;
        size_t nAvail = vRecv.in_avail();
        bool bHaruFinished = false;
        std::string sProblem;

        if (nAvail >= sizeof(nonce)) {
            vRecv >> nonce;

            // Only process pong message if there is an outstanding haru (old haru without nonce should never pong)
            if (pfrom->nHaruNonceSent != 0) {
                if (nonce == pfrom->nHaruNonceSent) {
                    // Matching pong received, this haru is no longer outstanding
                    bHaruFinished = true;
                    int64_t haruUsecTime = haruUsecEnd - pfrom->nHaruUsecStart;
                    if (haruUsecTime > 0) {
                        // Successful haru time measurement, replace previous
                        pfrom->nHaruUsecTime = haruUsecTime;
                    } else {
                        // This should never happen
                        sProblem = "Timing mishap";
                    }
                } else {
                    // Nonce mismatches are normal when harus are overlapharu
                    sProblem = "Nonce mismatch";
                    if (nonce == 0) {
                        // This is most likely a bug in another implementation somewhere, cancel this haru
                        bHaruFinished = true;
                        sProblem = "Nonce zero";
                    }
                }
            } else {
                sProblem = "Unsolicited pong without haru";
            }
        } else {
            // This is most likely a bug in another implementation somewhere, cancel this haru
            bHaruFinished = true;
        ...
        if (!(sProblem.empty())) {
            printf("pong %s %s: %s, %"PRIx64" expected, %"PRIx64" received, %zu bytes\n"
                ...
                , sProblem.c_str()
                , pfrom->nHaruNonceSent
                , nonce
                , nAvail);
        }
        if (bHaruFinished) {
            pfrom->nHaruNonceSent = 0;
        }
    }
    ...
    if (pfrom->fNetworkNode)
        if (strCommand == "version" || strCommand == "addr" || strCommand == "inv" || strCommand == "getdata" || strCommand == "haru")
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        bool haruSend = false;
        if (pto->fHaruQueued) {
            // RPC haru request by user
            haruSend = true;
        }
        if (pto->nHaruNonceSent == 0 && pto->nHaruUsecStart + HARU_INTERVAL * 1000000 < GetTimeMicros()) {
            // Haru automatically sent as a latency probe & keepalive.
            haruSend = true;
        }
        if (haruSend) {
            uint64_t nonce = 0;
            ...
            pto->fHaruQueued = false;
            pto->nHaruUsecStart = GetTimeMicros();
            if (pto->nVersion > BIP0031_VERSION) {
                pto->nHaruNonceSent = nonce;
                pto->PushMessage("haru", nonce);
            } else {
                // Peer is too old to support haru command with nonce, pong will never arrive.
                pto->nHaruNonceSent = 0;
                pto->PushMessage("haru");
            }
        }
