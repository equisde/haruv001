...
/** Time between harus automatically sent out for latency probing and keepalive (in seconds). */
static const int HARU_INTERVAL = 2 * 60;
/** Time after which to disconnect, after waiting for a haru response (or inactivity). */
...
public:
    ...
    // Haru time measurement:
    // The pong reply we're expecting, or 0 if no pong expected.
    uint64_t nHaruNonceSent;
    // Time (in usec) the last haru was sent, or 0 if no haru was ever sent.
    int64_t nHaruUsecStart;
    // Last measured round-trip time.
    int64_t nHaruUsecTime;
    // Whether a haru is requested.
    bool fHaruQueued;
...
public:
    ...
    void EndMessage() UNLOCK_FUNCTION(cs_vSend)
    {
        ...
        {
            printf("dropmessages DROPHARU SEND MESSAGE\n");
            AbortMessage();
            return;
        }
...
template<typename T1, typename T2, typename T3, typename T4, typename T5, typename T6, typename T7, typename T8, typename T9, typename T10>
    ...
    // Dropharu a node for sending stuff that is invalid
