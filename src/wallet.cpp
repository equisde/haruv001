...
int64_t CWallet::GetAnonDebit(const CTxIn& txin) const
{
    ...
    // -- amount of owned haru decreased
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && coin_type == ONLY_NONDENOMINATED_NOTMN) found = (pcoin->vout[i].nValue != GetMNCollateral()*COIN); // do not use MN funds 5,000 HARU
                } else {
                    found = true;
                }
                if(!found) continue;
...
// haru: total coins available for staking - WIP needs updating
        ...
        if(!masternodePayments.GetBlockPayee(pindexPrev->nHeight+1, payee)){
            int winningNode = GetCurrentMasterNode(1);
                if(winningNode >= 0){
                    ...
                    else burnAddress = "HARUXXXXXXXXXXXXXXXXXXXXXXXXXZeeDTw";
...
set< set<CTxDestination> > CWallet::GetAddressGrouharus()
{
    AssertLockHeld(cs_wallet); // mapWallet
    set< set<CTxDestination> > grouharus;
    set<CTxDestination> grouharu;
            ...
            BOOST_FOREACH(CTxOut txout, pcoin->vout)
                if (IsChange(txout))
                {
                    ...
                    if(!ExtractDestination(txout.scriptPubKey, txoutAddr))
                        continue;
                    grouharu.insert(txoutAddr);
                }
            }
            if (grouharu.size() > 0) {
                grouharus.insert(grouharu);
                grouharu.clear();
            }
        }
    ...
    set< set<CTxDestination>* > uniqueGrouharus; // a set of pointers to groups of addresses
    map< CTxDestination, set<CTxDestination>* > setmap;  // map addresses to the unique group containing it
    BOOST_FOREACH(set<CTxDestination> grouharu, grouharus)
    {
        ...
        set<CTxDestination>* merged = new set<CTxDestination>(grouharu);
        BOOST_FOREACH(set<CTxDestination>* hit, hits)
        {
            merged->insert(hit->begin(), hit->end());
            uniqueGrouharus.erase(hit);
            delete hit;
        }
        uniqueGrouharus.insert(merged);
    ...
    BOOST_FOREACH(set<CTxDestination>* uniqueGrouharu, uniqueGrouharus)
    {
        ret.insert(*uniqueGrouharu);
        delete uniqueGrouharu;
    }
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s HARU %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s HARU %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
bool CWallet::SendDnrToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    if (vNodes.empty())
    {
        sError = _("Error: Haru is not connected!");
        return false;
    };
...
bool CWallet::SendAnonToAnon(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    {
        sError = "Insufficient Anonymous HARU funds";
        return false;
    };
