...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <haruaddress> <account>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <haruaddress> <amount> [comment] [comment-to] [narration]\n" // Exchanges use the comments internally...
            "sendtoaddress <haruaddress> <amount> [narration]\n"
...
Value listaddressgrouharus(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            "listaddressgrouharus\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouharu, pwalletMain->GetAddressGrouharus())
    {
        Array jsonGrouharu;
        BOOST_FOREACH(CTxDestination address, grouharu)
        {
            Array addressInfo;
            ...
            jsonGrouharu.push_back(addressInfo);
        }
        jsonGrouharus.push_back(jsonGrouharu);
    }
    return jsonGrouharus;
}
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <haruaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <haruaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <haruaddress> [minconf=1]\n"
            "Returns the total amount received by <haruaddress> in transactions with at least [minconf] confirmations.");
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "Returns up to [count] most recent transactions skipharu the first [from] transactions for account [account].");
...
{
    // Make this thread recognisable as the key-topharu-up thread
    RenameThread("haru-key-top");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Haru server stopharu, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <haruaddress>\n"
            "Return information about <haruaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <harupubkey>\n"
            "Return information about <harupubkey>.");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid Haru stealth address."));
        return result;
    };
