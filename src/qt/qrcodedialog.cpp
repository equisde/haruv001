    ...
    address(addr)
{
    ...
    if (label.trimmed().isEmpty()) {
        setWindowTitle(QString("Haru - Generate QR Code"));
    } else {
        setWindowTitle(QString("Haru - Generate QR Code for %1").arg(label));
    }
...
{
    QString ret = QString("haru:%1").arg(address);
    int paramCount = 0;
...
{
    QString fn = GUIUtil::getSaveFileName(this, tr("Save QR Code"), QString(), tr("HARU Images (*.png)"));
    if (!fn.isEmpty())
