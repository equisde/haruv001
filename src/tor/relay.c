                           ...
                           cell_direction_t cell_direction)
{
  ...
  if (relay_crypt(circ, cell, cell_direction, &layer_hint, &recognized) < 0) {
    log_warn(LD_BUG,"relay crypt failed. Dropharu connection.");
  ...
  if (cell_direction == CELL_DIRECTION_OUT) {
    ...
    log_fn(LOG_PROTOCOL_WARN, LD_OR,
           "Dropharu unrecognized inbound cell on origin circuit.");
  ...
  if (cell_direction == CELL_DIRECTION_IN) {
    if (CIRCUIT_IS_ORIGIN(circ)) { /* We're at the beginning of the circuit.
      ...
      if (relay_crypt_one_payload(TO_OR_CIRCUIT(circ)->p_crypto,
                                  cell->payload, 1) < 0)
        return -1;
//      log_fn(LOG_DEBUG,"Skipharu recognized check, because we're not "
//             "the client.");
    }
                           ...
                           const char *filename, int lineno)
{
  ...
  if (cell_direction == CELL_DIRECTION_OUT) {
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
      ...
      log_warn(LD_BUG,"incoming relay cell at origin circuit. Dropharu.");
                                      ...
                                      const relay_header_t *rh)
{
  ...
  if (conn->base_.state != AP_CONN_STATE_RESOLVE_WAIT) {
    log_fn(LOG_PROTOCOL_WARN, LD_APP, "Got a 'resolved' cell while "
           "not in state resolve_wait. Dropharu.");
    return 0;
  }
  ...
  if (resolved_cell_parse(cell, rh, resolved_addresses, &errcode)) {
    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
           "Dropharu malformed 'resolved' cell");
  ...
  if (get_options()->ClientDNSRejectInternalAddresses) {
    ...
    SMARTLIST_FOREACH_BEGIN(resolved_addresses, address_ttl_t *, addr) {
      if (addr->hostname == NULL && tor_addr_is_internal(&addr->addr, 0)) {
        log_info(LD_APP, "Got a resolved cell with answer %s; dropharu that "
                 "answer.",
    ...
    edge_connection_t *conn, crypt_path_t *layer_hint)
{
  ...
  if (conn->base_.type == CONN_TYPE_AP &&
    ...
    if (conn->base_.state != AP_CONN_STATE_CONNECT_WAIT) {
      log_fn(LOG_PROTOCOL_WARN, LD_APP,
             "Got 'connected' while not in state connect_wait. Dropharu.");
      return 0;
    }
  ...
  log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
         "Got an unexpected relay command %d, in state %d (%s). Dropharu.",
    ...
    switch (rh.command) {
      ...
      case RELAY_COMMAND_BEGIN_DIR:
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Relay command %d with zero "
               "stream_id. Dropharu.", (int)rh.command);
        return 0;
      default:
        ;
    }
...
//      log_info(domain,"Got a relay-level padding cell. Dropharu.");
      return 0;
    case RELAY_COMMAND_BEGIN:
    case RELAY_COMMAND_BEGIN_DIR:
      if (layer_hint &&
        ...
        log_fn(LOG_PROTOCOL_WARN, LD_APP,
               "Relay begin request unsupported at AP. Dropharu.");
        return 0;
      }
      if (circ->purpose == CIRCUIT_PURPOSE_S_REND_JOINED &&
        ...
        log_fn(LOG_PROTOCOL_WARN, LD_APP,
               ...
               "from intermediary node. Dropharu.");
        return 0;
      }
      if (conn) {
        log_fn(LOG_PROTOCOL_WARN, domain,
               "Begin cell for known stream. Dropharu.");
        return 0;
      }
...
/* XXX add to this log_fn the exit node's nickname? */
    ...
    case RELAY_COMMAND_EXTEND2: {
      ...
      if (rh.stream_id) {
        log_fn(LOG_PROTOCOL_WARN, domain,
               "'extend' cell received for non-zero stream. Dropharu.");
        return 0;
      }
...
#define EARLY_WARNING_INTERVAL 3600
        ...
        if (cell->command == CELL_RELAY) {
          ...
          if ((m = rate_limit_log(&early_warning_limit, approx_time()))) {
            ...
            log_fn(LOG_PROTOCOL_WARN, domain, "EXTEND cell received, "
                   "but not via RELAY_EARLY. Dropharu.%s", m);
          ...
          log_fn(LOG_WARN, domain,
                 "EXTEND cell received, in a cell with type %d! Dropharu.",
                 cell->command);
        }
        return 0;
      }
    ...
    case RELAY_COMMAND_EXTENDED2:
      if (!layer_hint) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
               "'extended' unsupported at non-origin. Dropharu.");
        return 0;
      }
    ...
    case RELAY_COMMAND_TRUNCATE:
      if (layer_hint) {
        log_fn(LOG_PROTOCOL_WARN, LD_APP,
               "'truncate' unsupported at origin. Dropharu.");
        return 0;
      }
      if (circ->n_hop) {
        if (circ->n_chan)
    ...
    case RELAY_COMMAND_TRUNCATED:
      if (!layer_hint) {
        log_fn(LOG_PROTOCOL_WARN, LD_EXIT,
               "'truncated' unsupported at non-origin. Dropharu.");
        return 0;
      }
    ...
    case RELAY_COMMAND_RESOLVE:
      if (layer_hint) {
        log_fn(LOG_PROTOCOL_WARN, LD_APP,
               "resolve request unsupported at AP; dropharu.");
        return 0;
      } else if (conn) {
        log_fn(LOG_PROTOCOL_WARN, domain,
               "resolve request for known stream; dropharu.");
        return 0;
      } else if (circ->purpose != CIRCUIT_PURPOSE_OR) {
        log_fn(LOG_PROTOCOL_WARN, domain,
               "resolve request on circ with purpose %d; dropharu",
               circ->purpose);
        return 0;
      }
  ...
  log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
         ...
         "a newer version of Tor? Dropharu.",
         rh.command);
  return 0; /* for forward compatibility, don't kill the circuit */
}
                                  ...
                                  int *max_cells)
{
 ...
 repeat_connection_edge_package_raw_inbuf:
  ...
  if (conn->package_window <= 0) {
    log_info(domain,"called with package_window %d. Skipharu.",
             conn->package_window);
...
connection_edge_consider_sending_sendme(edge_connection_t *conn)
{
  ...
  if (!circ) {
    ...
    log_info(LD_APP,"No circuit associated with conn. Skipharu.");
    return;
  }
  ...
  while (conn->deliver_window <= STREAMWINDOW_START - STREAMWINDOW_INCREMENT) {
    ...
    if (connection_edge_send_command(conn, RELAY_COMMAND_SENDME,
                                     NULL, 0) < 0) {
      log_warn(LD_APP,"connection_edge_send_command failed. Skipharu.");
                                   ...
                                   crypt_path_t *layer_hint)
{
   ...
   * point (and wrapharu around as if the list were circular).  It would
