// Copyright (c) 2014 The Haru developers
...
void ThreadSecureMsg(void* parg)
{
    ...
    RenameThread("haru-smsg"); // Make this thread recognisable
...
void ThreadSecureMsgPow(void* parg)
{
    ...
    RenameThread("haru-smsg-pow"); // Make this thread recognisable
...
int SecureMsgBuildBucketSet()
{
    ...
    */
    ...
    for (fs::directory_iterator itd(pathSmsgDir) ; itd != itend ; ++itd)
    {
        ...
        {
            printf("Dropharu file %s, expired.\n", fileName.c_str());
            try {
                fs::remove((*itd).path());
            } catch (const fs::filesystem_error& ex)
            {
        ...
        {
            if (fDebugSmsg)
                printf("Skipharu wallet locked file: %s.\n", fileName.c_str());
            continue;
        };
...
bool SecureMsgShutdown()
{
    ...
    printf("Stopharu secure messaging.\n");
...
bool SecureMsgEnable()
{
    ...
    // -- haru each peer, don't know which have messaging enabled
    {
        LOCK(cs_vNodes);
        BOOST_FOREACH(CNode* pnode, vNodes)
        {
            pnode->PushMessage("smsgHaru");
            pnode->PushMessage("smsgPong"); // Send pong as have missed initial haru sent by peer when it connected
        };
    }
...
bool SecureMsgReceiveData(CNode* pfrom, std::string strCommand, CDataStream& vRecv)
{
    ...
    */
    ...
    if (strCommand == "smsgHaru")
    {
        // -- smsgHaru is the initial message, send reply
...
bool SecureMsgSendData(CNode* pto, bool fSendTrickle)
{
    ...
    */
    ...
    if (pto->smsgData.lastSeen == 0)
    {
        ...
        // -- Send smsgHaru once, do nothing until receive 1st smsgPong (then set fEnabled)
        pto->PushMessage("smsgHaru");
...
int SecureMsgWalletUnlocked()
{
    ...
    */
    ...
    for (fs::directory_iterator itd(pathSmsgDir) ; itd != itend ; ++itd)
    {
        ...
        if (fileTime < now - SMSG_RETENTION)
        {
            printf("Dropharu wallet locked file %s, expired.\n", fileName.c_str());
            try {
                fs::remove((*itd).path());
            } catch (const boost::filesystem::filesystem_error& ex)
            {
